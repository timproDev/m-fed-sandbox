			...
			for (var i = 0; i < aGa.length; i++) {
				...
				aGa[i].addEventListener('click', function(event){					
					...
					if (hasClass(this, 'has-sub') == true) {
						...
						parentVal = this.innerHTML;
						var elIndexOne = parentVal.indexOf("</span>");
						elIndexOne = elIndexOne + 7;
						var elIndexLast = parentVal.indexOf("<div class=\"tooltip\">");					
						parentVal = parentVal.substring(elIndexOne, elIndexLast);
					// if link is not dropdown
					} else if (!hasClass(this, 'has-sub') == true && this.parentNode.nodeName != 'LI') {
						
						// IE fix
						thisValue = this.innerHTML;
						var elIndexOne = thisValue.indexOf("</span>");
						elIndexOne = elIndexOne + 7;
						var elIndexLast = thisValue.indexOf("<div class=\"tooltip\">");					
						thisValue = thisValue.substring(elIndexOne, elIndexLast);
