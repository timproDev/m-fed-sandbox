...
(function(window, document, $, undefined) {
    ...
    function position(error, $target) {
		...
		//Custom else if for contact us form. Specific to Marsh site
		} else if (filters.custom.test(location)) {
			//error.css("right", "15px");
			error.css("position", "relative");
			//if (location === 'custom-contact') {
            	//console.log("third if" + error.parent().find('label').width());
                //error.offset({top: offset.top - error.outerHeight() - 2});
           // }          
        }        
    }
